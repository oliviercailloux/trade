...
\begin{itemize}
  \item Prices of goods produced and sold in a given country are still $p = p_D = p_D^{(0)} = p_D^{(1)} = p_X = p_X^{(0)} = p_X^{(1)} = \frac{σ}{σ - 1} \frac{w}{φ}$
  \item Iceberg type trading cost: goods sold in the other country have price $p_{D → X} = p_{D → X}^{(1)} = τ p_D$, with $1 ≤ τ$
  \item Total production of a good produced in $D$ becomes $q_D^{(1)} = q_{D → D} + τ q_{D → X}$, with $q_{D → X}$ the quantity that effectively arrives abroad
