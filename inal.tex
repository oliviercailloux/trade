...
\begin{itemize}
  ...
  \item Revenue constraint thes $w = \p_i c_i$
  \it^\text{country} From the agrangian we obtain $\frac{σ}{σ - 1} U(\bar{c})^\frac{1}{σ} \frac{σ - 1}{σ} c_i^{\frac{σ - 1}{σ} - 1} - λ p_i = 0$ (because $\frac{σ}{σ - 1} - 1 = \frac{σ}{σ - 1}\frac{1}{σ}$), thus $c_i = λ^{-σ} p_i^{-σ} U(\bar{c})$
  ...
  \item We obtain elasticity of demand $ε_i = \frac{∂q_i}{∂p_i} \frac{p_i}{q_i} = \left(-σ q_i (\frac{p_i}{P})^{-1} \frac{∂}{∂p_i} (\frac{p_i}{P})\right) \frac{p_i}{q_i}$, and, approximating $P$ as constant with respect to $p_i$, $ε_i ≈ -σ$
  ...
  \item We have $U_j(\bar{q}) = \frac{Lw}{p} n^\frac{1}{σ - 1}$ thus $\frac{∂ \log U}{∂ \log n} = \frac{1}{σ - $
  \icem Why “Welare increases at a rate in{itemize}
...
\begin{itemize}
  \item Iceberg type trading cost: 
\end{itemize}
...
[widest={\bfseries{}SUBJECT-Assessment-WI
Country $D$ (with variable levels denoted as $v_D^{(0)}$) opens up and starts trading with country $X$ (with variable levels denoted as $v_X^{(0)}$); same parameters except for sizes $L_D$ and $L_X$; resulting variable levels denoted as $v_D^{(1)}$
\NewDocumentCommand{\jbar}{}{\overline{j}}
\begin{itemize}
  \item Prices of goods produced and sold in a given country $j ∈ \set{D, X}$ are still $p_j = p_j^{(0)} = p_j^{(1)} = \frac{σ}{σ - 1} \frac{w_j}{φ}$
  \item Iceberg type trading cost: goods sold in the other country have price $p_{j → \jbar} = p_{j → \jbar}^{(1)} = τ p_j$, with $1 ≤ τ$
  \item Total production of a good produced in $j$ becomes $q_j^{(1)} = q_{j → j} + τ q_{j → \jbar}$, with $q_{j → \jbar} = q_{j → \jbar}^{(1)}$ the quantity that effectively arrives abroad
  \item Thus the enterprise sells $q_j^{(1)}$ at price $p_j$, equivalently, $q_{j → j}$ at price $p_j$ domestically and $q_{j → \jbar}$ at price $τ p_j$ abroad
  \item It makes profit $π_j^{(1)} = p_j q_j^{(1)} - w_j (f + \frac{q_j^{(1)}}{φ}) = \frac{1}{σ - 1} \frac{w_j q_j^{(1)}}{φ} - w_j f$
  \item From $π_j^{(1)} = 0$ we obtain $q_j = q_j^{(1)} = φ f (σ - 1) = q_j^{(0)}$
  \item Using $L_j = n_j^{(1)} f + n_j^{(1)} \frac{q_j^{(1)}}{φ}$, we obtain $n_j = n_j^{(1)} = \frac{L_j}{σ f} = n_j^{(0)}$
  \item $P_j^{(1)} = [n_j {p_j}^{1 - σ} + n_{\jbar} {(τ p_{\jbar})}^{1 - σ}]^\frac{1}{1 - σ}$
  \item With $τ = 1$ and $n_j = n_{\jbar}$, we get $P_j^{(1)} = \frac{P_j^{(0)}}{2^\frac{1}{σ - 1}}$: price index decreases
  \item No pro-competitive effect: $σ$ fixed, does not contribute to lowering prices
  \item From above, $q_{j → \jbar} = \left(\frac{p_{j → \jbar}}{P_{\jbar}^{(1)}}\right)^{-σ} \frac{w_{\jbar} L_{\jbar}}{P_{\jbar}^{(1)}}$ (correct?), thus $q_{j → \jbar} = \left(\frac{τ p_j}{P_{\jbar}^{(1)}}\right)^{-σ} \frac{w_{\jbar} L_{\jbar}}{P_{\jbar}^{(1)}}$
  ...
  \item From trade balance $X_j = X_{\jbar}$, $\frac{p_j}{p_{\jbar}} = \frac{w_j}{w_{\jbar}}$, $\frac{n_j}{n_{\jbar}} = \frac{L_j}{L_{\jbar}}$ and $\frac{q_{j → \jbar}}{q_{\jbar → j}} = \frac{w_j^{-σ} w_{\jbar} L_{\jbar} {P_j^{(1)}}^{1 - σ}}{w_{\jbar}^{-σ} w_j L_j {P_{\jbar}^{(1)}}^{1 - σ}}$
  follows $\frac{X_j}{X_{\jbar}} = 1 = \frac{w_j^{-σ} {P_j^{(1)}}^{1 - σ}}{w_{\jbar}^{-σ} {P_{\jbar}^{(1)}}^{1 - σ}}$,
  whence $\frac{w_j}{w_{\jbar}} = \left(\frac{P_j^{(1)}}{P_{\jbar}^{(1)}}\right)^\frac{1 - σ}{σ}$
  \item With $τ = 1$, we get $P_j^{(1)} = P_{\jbar}^{(1)}$ and thus $w_j = w_{\jbar}$.
  \item With $τ = ∞$, 
  we get $\left(\frac{P_j^{(1)}}{P_{\jbar}^{(1)}}\right)^{1 - σ} 
  = \frac{n_{\jbar}}{n_j} \left(\frac{p_{\jbar}}{p_j}\right)^{1 - σ}
  = \big(\frac{w_j}{w_{\jbar}}\big)^σ
  = \frac{L_{\jbar}}{L_j} \left(\frac{w_{\jbar}}{w_j}\right)^{1 - σ}$ 
  thus $\frac{w_j}{w_{\jbar}} 
  = \left(\frac{L_{\jbar}}{L_j}\right)^\frac{1}{σ} \left(\frac{w_{\jbar}}{w_j}\right)^\frac{1 - σ}{σ}$
\end{itemize}
...
\begin{itemize}
  ...
  \item Producer: $w^{(0)} = 1$, $q = 1$
  \item Solving: $n_j = L_j$, $P_j^{(0)} = 1/L_j$
  \item Consumer: $c_j = 1/L_j$, $U(c_j) = L_j$
  \item Set $L_D = 1$ and $L_X = 2$
  \item $n_D = P_D^{(0)} = c_D = U(c_D) = 1$
  \item $n_X = U(c_X) = 2$, $P_X^{(0)} = c_X = 1/2$
  \item $p_{j → \jbar} = τ$
  \item $P_D^{(1)} = 1/(1 + 2/τ) = τ/(τ+2)$
  \item $P_X^{(1)} = 1/(2 + 1/τ) = τ/(2τ+1)$
\end{itemize}
